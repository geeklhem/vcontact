...
def split_dataset(dataset, criterion, fold):
    ...
    cv = sklearn.cross_validation.StratifiedKFold(dataset[criterion], fold)
...
def classification_metrics(dataset,
    ...
    same wieght in the final result regardless of its size. 
    ...
    if out["reference_classes"]: 
        pred_count = dataset.groupby(pred).count()[pred]
